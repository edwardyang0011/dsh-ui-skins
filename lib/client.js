window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		Schema.object({ [SKIN_FIELD]: Schema.union([...SKIN_IDS, DEFAULT_SKIN]).default(DEFAULT_SKIN) });
		//#endregion
		...
		//#endregion
		//#region lib/types/client/SkinRow.js
		/**
		...
		function SkinRow({ t, setSkin, useStore }) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [(0, react_jsx_runtime.jsx)("div", {
					...
					children: CHOICES.map(({ id, labelKey }) => (0, react_jsx_runtime.jsxs)("button", {
						type: "button",
		...
		*/
		function createSkinRowStore() {
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				...
				actions: { sync: (d, selected, revision) => {
		...
		*/
		function apply(ctx) {
			...
			const adopt = () => {
				const skin = host.getSnapshot().value?.skin ?? "default";
			...
			* preference (the Appearance row won) clears it and forgets the fallback. */
			const sync = (snapshot) => {
				...
				const persisted = host.getSnapshot().value?.skin ?? "default";
			...
			const injected = (actions) => {
				...
				return { setSkin: (id) => {
					if (id !== "default" && !isSkinId(id)) throw new Error(`skin "${id}" is not registered`);
