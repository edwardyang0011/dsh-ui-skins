...
export function createSkinRowStore(): EngineStoreHandle<SkinRowState, SkinRowActions> {
  return defineStore({
    init: (): SkinRowState => ({ selected: 'default', custom: null, revision: -1 }),
    actions: {
      sync: (d, selected: string, custom: CustomSkinColors | null, revision: number) => {
        if (revision <= d.revision) return
        d.selected = selected
        d.custom = custom
        d.revision = revision
      },
      setCustom: (d, custom: CustomSkinColors) => {
        d.custom = custom
      },
    },
  })
